...
if __name__ == '__main__':
    try:
        ...
        if config_data['run_exp']:
            X, y = s_instance.load_data_and_save_X_Y(config_data['class0_dir'], config_data['class1_dir'])
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)

            s_instance.run_exp_varying_training_datasize(X_train, y_train, X_test, y_test)
            exit()

        if config_data['solve_maze']:
            s_instance.load_maze_scan_predict('maze.png')
            exit()
            ...
            if config_data['use_pretrained']:
                ...
                history = s_instance.sim_train(X_train, y_train, X_test, y_test, model=config_data['model_name'],
                                               ...
                                               validation_split=0.2, early_stopping=False, save=True,
